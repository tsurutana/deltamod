...
public class OBJStream {
	...
	public static Model3D load (String filepath) {
		...
		InputStream is;
		Vertex3D minV = new Vertex3D(Double.MAX_VALUE, Double.MAX_VALUE, Double.MAX_VALUE);
		Vertex3D maxV = new Vertex3D(-Double.MAX_VALUE, -Double.MAX_VALUE, -Double.MAX_VALUE);
			...
			while (token != StreamTokenizer.TT_EOF) {
				if(st.ttype == StreamTokenizer.TT_WORD) {
					if(st.sval.equals("v")) { // vertex
						st.nextToken(); double x = st.nval; // x
						st.nextToken(); double y = st.nval; // y
						st.nextToken(); double z = st.nval; // z
	...
	public static boolean save (String filepath, Model3D model) {
		...
		try {
			...
			for (Vertex3D v : m.vertices){
				v.index = i++;
				bw.write("v "+ df.format(v.p.x) +" "+ df.format(v.p.y) +" "+ df.format(v.p.z));
				bw.newLine();
			}
